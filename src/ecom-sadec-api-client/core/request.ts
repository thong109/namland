...
export const getHeaders = async <T>(
  ...
  if (
    ...
    localStorage !== null &&
    localStorage.getItem(AuthConstant.LocalStorageAuthKey)
  ) {
    headers['Authorization'] =
      `Bearer ${JSON.parse(localStorage.getItem(AuthConstant.LocalStorageAuthKey))?.state?.token}`;
  }

  const currentLocale = await getCookie('NEXT_LOCALE');

  headers['App.Culture'] = currentLocale || 'vi';
