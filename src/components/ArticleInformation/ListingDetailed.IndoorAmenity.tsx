...
interface ListingDetailedIndoorAmenityProps {
  locale: string;
  listingDetail: listingPropertyModel;
}

const ListingDetailedIndoorAmenity: React.FC<ListingDetailedIndoorAmenityProps> = ({
  locale,
  listingDetail,
}) => {
  ...
  const indoorAmenities = useMemo(
    () => listingDetail?.inDoorAmenities ?? [],
    [listingDetail, listingDetail?.inDoorAmenities],
  );

  return (
    <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
      {indoorAmenities && indoorAmenities.length ? (
...
export default React.memo(ListingDetailedIndoorAmenity);
