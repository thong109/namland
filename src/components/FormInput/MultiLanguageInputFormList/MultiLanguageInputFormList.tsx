...
const MultiLanguageInputFormList: React.FC<MultiLanguageTextAreaProps> = ({
  value = [],
  onChange,
  maxLength,
  disabled = false,
  ...
  const [languageValue, setLanguageValue] = useState(
    Object.fromEntries(value.map((item) => [item.language, item.value])),
  );
  ...
  useEffect(() => {
    if (previousValue && !isEqual(previousValue, value)) {
      setLanguageValue(Object.fromEntries(value.map((item) => [item.language, item.value])));
    }
  }, [value]);
...
export default MultiLanguageInputFormList;
