...
import { moduleConversation } from '@/libs/appconst';
...
const NewHomeInquiryDetailStaff: FC<Props> = ({ params }) => {
  ...
  const getDetail = async (id: string) => {
    if (params.id) {
      try {
        const data = await apiTicketManagement.getInquryNewHomeByMe(id);
  ...
  const onSubmitMessage = async (file?: File) => {
    ...
    if (values || file) {
      const body = {
        ...
        moduleId: moduleConversation.newHome,
      };

      try {
        await apiConversationService.createConversationNEwHomeInquiry(body, file);
  ...
  const renderActions = () => {
    return (
      <div className="flex justify-end">
        <ButtonBack text={t('goBack')} onClick={() => push('/client-inquiry-client')} />
      </div>
    );
  };
