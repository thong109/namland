...
import { align, listPackage, listStatusProject, renderDateTimeV2 } from '@/libs/appconst';
...
const HistoryUsePush = (props: IProps) => {
  ...
  useEffect(() => {
    if (props.activeKey === props.tabKey) {
      getMyListingPushLog(filter);
    }
  }, [props.activeKey, props.tabKey]);

  const getMyListingPushLog = async (filter) => {
    const responseData: any = await transactionApiInAdmin.GetMyListingPushLog(filter);
  ...
  const handleSearch = async (name, value) => {
    if (name === 'date') {
      ...
      getMyListingPushLog(convertFilterDate(filter, value));
    } else {
      const newFilter = { ...filter, [name]: value };
      setFilter(newFilter);
      getMyListingPushLog(newFilter);
    }
  };
  ...
  const columnHistory = [
    ...
    },
    {
      ...
      ellipsis: true,
      render: (numOfPush) => {
        return <div className="text-xs">{numOfPush}</div>;
      },
    },
    {
