...
const ListingPackagePage = (props: IProps) => {
  ...
  useEffect(() => {
    if (props.activeKey === props.tabKey) {
      getListTransactionListPackage();
    }
  }, [filter, props.activeKey, props.tabKey]);

  const getListTransactionListPackage = async () => {
    const responseData: any = await transactionApiInAdmin.getListTransactionListPackage(filter);
  ...
  const handleExport = () => {
    ...
    startTransition(async () => {
      await transactionApiInAdmin.exportMyTransactionPackage(body);
    });
  };
