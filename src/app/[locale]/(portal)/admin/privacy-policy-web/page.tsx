...
import { PreviewTsAndCsWebComponents } from './components/PreviewWeb';
...
const PagePrivacyPolicyWeb: FC<Props> = ({ params }) => {
  ...
  const getDetail = async () => {
    const data = await TsAndCsService.getData({ type: TypeModuleEnum.PrivacyPolicy });
    setDataTsAndCs(data);
    if (data === null) {
  ...
  return userInfo?.type === UserTypeConstant.Salesman &&
    ...
    <>
      <AppPageMeta title={`${t('EcomLeftMenuBarPrivacyPolicy')}`} />
...
export default PagePrivacyPolicyWeb;
