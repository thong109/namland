...
import { eyeIcon, inEyeIcon } from '@/libs/appComponents';
...
const RatingAgent: React.FC = () => {
  ...
  const getAllRatingAgent = async (dataFilter) => {
    const responseData: any = await ratingApiService.getAllRatingAdmin(dataFilter);
  ...
  const changeStatus = (isShow, id) => {
    Modal.confirm({
      content: (
        <div>
          {t('EcomRatingChangeStatusContent')}
          <Form form={formReason} layout="vertical" size="middle">
            <Form.Item
              rules={[
                {
                  required: true,
                  message: `${eForm('pleaseInput')} ${t('EcomPropertyListingRejectReason')}`,
                },
              ]}
              name="reasonToHide"
            >
      ...
      title: t('EcomRatingChangeStatusTitle'),
      okText: t('YES'),
      cancelText: t('NO'),
      centered: true,
      okType: 'default',
      async onOk() {
        await formReason.validateFields();
        const values = formReason.getFieldsValue();

        await ratingApiService.activeOrInActive({ isShow, id, reasonToHide: values?.reasonToHide });
        getAllRatingAgent(filter);
  ...
  const columnTopup = [
      ...
      render: (user) => {
        return (
          <div className="text-xs">
            ...
            {user?.email}
            <br />
            {convertPhoneNumber84To0(user?.phone)}
          </div>
        );
      },
    },
    ...
    {
      title: t('EcomPropertyListingPageListViewAction'),
      dataIndex: 'isShow',
      key: 'isShow',
      width: 90,
      align: align.center,
      render: (isShow, row) => (
        <Button size="small" onClick={() => changeStatus(isShow, row?.id)}>
          {isShow ? eyeIcon : inEyeIcon}
        </Button>
      ),
    },
  ];
