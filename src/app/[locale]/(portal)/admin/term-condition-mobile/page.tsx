...
import { PreviewTsAndCsMobileComponents } from './components/PreviewTsAndCsMobileComponents';
...
const TsAndCsPageMobile: FC<Props> = ({ params }) => {
  ...
  const getDetail = async () => {
    const data = await TsAndCsService.getData({ type: TypeModuleEnum.TermsCondition });
    setDataTsAndCs(data);
    if (data === null) {
  ...
  const onSave = async () => {
    ...
    const body = {
      ...
      isActive: true,
      type: TypeModuleEnum.TermsCondition,
    };
  ...
  return userInfo?.type === UserTypeConstant.Salesman &&
    ...
    <>
      ...
      <PreviewTsAndCsMobileComponents
        data={dataPreview}
...
export default TsAndCsPageMobile;
