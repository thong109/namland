...
const LineChartComponent: FC<Props> = ({ dataBeforConvert, title }) => {
      ...
      monthData.data.forEach((item) => {
        ...
        if (existingCategory) {
          existingCategory.data.push({
            [`month${monthData.month}`]: item.price,
          });
        } else {
          acc.push({
            listingCategoryName: item.listingCategoryName,
            data: [{ [`month${monthData.month}`]: item.price }],
          });
        }
      });
  ...
  return (
    <>
      <label className="text-lg font-semibold text-portal-textTitleChart">{title}</label>
      <Line height={70} className="w-full" options={options} data={data} />
    </>
  );
};
